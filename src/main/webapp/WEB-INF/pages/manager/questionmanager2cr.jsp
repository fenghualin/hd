...
<table id="questionTable2cr"></table>
<script type="text/javascript">
	if (typeof (questions2cr) != "undefined") {
		questions2cr.init();
	} else {
		var questions2cr = {
			createTable : function() {
				KX.createTable({
					id : 'questionTable2cr',
					url : '<%=path%>/questionmanager/questiondata2cr',
					columns : [ [ {
						field : 'id',
						title : 'id',
						hidden : true
...
// 									alert(questionkinds+"--"+questionleirong+"--"+answerA+"--"+pointA+"--"+answerB+"--"+pointB+"--"+answerC+"--"+pointC+"--"+answerD+"--"+pointD+"--"+answerE+"--"+pointE);
									...
									$.post("<%=path%>/questionmanager/questionsave2cr",{"questionkinds":questionkinds,"questionleirong":questionleirong,"answerA":answerA,"answerB":answerB,"answerC":answerC,"answerD":answerD,"answerE":answerE,"pointA":pointA,"pointB":pointB,"pointC":pointC,"pointD":pointD,"pointE":pointE,"questioninverted":questioninverted},function(data){
										if(data=="ok"){
											$.messager.alert('操作提示','问题添加成功！','info');
											KX.refreshTable('questionTable2cr');
											KX.closeDialog();
										}
										else{
					...
					deleteFuc : function(rowDatas) {
						$.messager.confirm('操作提示', '确认删除该问题？', function(r){
							if(r){
								...
								$.post("<%=path%>/questionmanager/questiondelete2cr", {"qid" : ids.substring(1)},function(data) {
									if (data=="ok") {
					...
					viewFuc : function(rowIndex, rowData) {
						KX.showDialog(600, 500, "查看", "<%=path%>/questionmanager/questionlook2cr?qid="+rowData.id, "查看问题类容", "search",
							[ {
								text: '取消',
								iconCls: 'icon-cancel',
								handler: function(){
					...
					modifyFuc : function(rowIndex, rowData) {
						KX.showDialog(600, 500, '修改问题', "<%=path%>/questionmanager/questionupdatepage2cr?qid="+rowData.id, '问题类容',
							'search', [ {
								text : '修改',
								iconCls : 'icon-ok',
								handler : function() {
