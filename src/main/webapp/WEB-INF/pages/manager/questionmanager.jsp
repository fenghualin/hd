			...
			createTable : function() {
				KX.createTable({
						...
						KX.showDialog(600, 500, '添加问题', 'questionaddui', '添加问题', 'add',
							[ {
								...
								handler : function() {
									...
									$.post("questionsave",{"questionkinds":questionkinds,"questionleirong":questionleirong,"answerA":answerA,"answerB":answerB,"answerC":answerC,"answerD":answerD,"pointA":pointA,"pointB":pointB,"pointC":pointC,"pointD":pointD},function(data){
										if(data=="ok"){
											$.messager.alert('操作提示','问题添加成功！','info');
											KX.refreshTable('questionTable');
											KX.closeDialog();
										}
										else if(data=="no"){
					...
					deleteFuc : function(rowDatas) {
						$.messager.confirm('操作提示', '确认删除该问题？', function(r){
							if(r){
								...
								$.post("questiondelete", {"qid" : ids.substring(1)},function(data) {
									if (data=="ok") {
					...
					viewFuc : function(rowIndex, rowData) {
						KX.showDialog(600, 400, "查看", "questionlook?qid="+rowData.id, "查看问题类容", "search",
							[ {
								text: '取消',
								iconCls: 'icon-cancel',
								handler: function(){
					...
					modifyFuc : function(rowIndex, rowData) {
						KX.showDialog(600, 400, '修改问题', "questionupdatepage?qid="+rowData.id, '问题类容',
							'search', [ {
								text : '修改',
								iconCls : 'icon-ok',
								handler : function() {
...
// 									alert(answerB+"--"+answerA+"--"+answerC+"--"+answerD+"--"+pointA+"--"+pointB);
									...
									$.post("questionUpdate",{"questionid":questionid,"questionkinds":questionkinds,"questionleirong":questionleirong,"answerA":answerA,"answerB":answerB,"answerC":answerC,"answerD":answerD,"pointA":pointA,"pointB":pointB,"pointC":pointC,"pointD":pointD},function(data){
										if(data=="ok"){
