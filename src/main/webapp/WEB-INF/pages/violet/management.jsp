...
<style>
body{ background:url(${stc }images/zzgl/zzgl2.jpg) no-repeat center center;}
.tips{}
.tips .bd{ color:#000;}
.time{ font-size:12px;position:absolute; left:5px; top:5px; z-index:10;}
.qusshows{ width:910px; height:280px; font-size:14px; position:absolute;text-align:center; left:50%; top:50%; margin-left:-405px; margin-top:-120px;  display:none;}
.qusshows .bd{ padding:25px 90px 0 90px; line-height:1.6; text-align:left;color:#181818;}
.qusshows .title{ margin-bottom:40px;width:630px;}
.qusshows .quslist{width:630px;}
...
function daansubmit(student_choice,option_score){
		...
		time=time2;
		$.post("<%=path%>/green/TnteractionA/linguisiticSave",
			{"option_score":option_score,"question_no":question_no,"student_choice":student_choice,"reaction_time":reaction_time,"test_no":test_no,"kinds":1});
		ajaxquestion();
}
...
function sendAjaxQuestion(pagestart, pagesize) {
	$.ajax({
		type: "POST",
		url: "/violet/induction/Questiondatas",
...
<div class="fullscreen" id="swf1">
			<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="1000" height="750" id="FlashID">
				<param name="movie" value="${stc }swf/zzgl.swf">
				...
				<!--[if !IE]>-->
				<object data="${stc }swf/zzgl.swf" type="application/x-shockwave-flash" width="1000" height="750" id="FlashID2" align="absmiddle">
						<!--<![endif]-->
						<param name="quality" value="high">
						<param name="wmode" value="opaque">
						<param name="swfversion" value="7.0.70.0">
